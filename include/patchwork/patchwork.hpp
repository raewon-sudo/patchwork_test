...
void PatchWork<PointT>::estimate_ground(
    ...
    for (int i  = 0; i < laserCloudIn.points.size(); i++)             //Check z point nao nho hon gia trị k * sensor_height với k = -1.8
    {
    ...
    laserCloudIn.points.erase(laserCloudIn.points.begin(), it);       //Xóa toàn bôj các pointcloud có giá trị z thỏa mãn phía trên
